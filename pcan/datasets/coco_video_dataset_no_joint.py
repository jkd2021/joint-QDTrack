...
class CocoVideoDatasetNoJoint(CocoDataset):
    ...
    def evaluate(self,
        ...
        res_total = {}
        res_total["bbox/segm mAP"] = {}
        res_total["track bbox/segm mAP"] = {}
        res_total["per video segm mAP"] = {}

        # # mAP of bbox/segm results (not tracked results)
        # print("\n\nstart calculating mAP of bbox/segm results\n")
        # print("\nthat's not track res, only the segm res in the track res below is for segm performance comparison\n")
        ...
        #     res_total["bbox/segm mAP"] = super_eval_results


        # mAP of bbox/segm in track results (frame-level evaluation)
        if "amodal" in self.ann_file:
            ...
            print("ERROR: no 'amodal' or 'visible' in ann_file")
            exit()
        ...
        if super_metrics:
            if 'bbox' in super_metrics and 'segm' in super_metrics:
                ...
                print("\nstart to transfer track res into bbox/segm res")
                start = time.time()
            ...
            super_eval_results = SAILVOS.evaluate(
                ...
                metric_items=metric_items)
            eval_results.update(super_eval_results)
            res_total["track bbox/segm mAP"] = super_eval_results


        # MOTS results, can be omitted
        ...
        ## evaluate the mAP of VIS (video-level evaluation)
        ...
        if 'segm' in metrics:
            ...
            print("start to transfer per img res into per vid res")
            start = time.time()
            video_id = 1
            res_ytvis = []
            res_ytvis_video = []
            ...
            for j in range(len(res_ytvis)):
                ...
                if check_none_video == 0:  ## empty video without any instance
                    print("video {} is empty".format(j + 1))
                    pass
                        ...
                        for key in res_ytvis[j][m].keys():  # here key is the ins_id in one video
                            if key not in map_instance_cat.keys():
                                ...
                                if map_instance_cat[key] != int(res_ytvis[j][m][key]["label"] + 1):
                                    print("ERROR: in video {}, identical ins with diff cat".format(j + 1))
                    ...
                    if len(set_ins_id) != instance_max + 1:
                        print("ERROR: in video {}, the instance ids in vid are not continuous".format(j + 1))
                    ...
                    for n in range(instance_max + 1):  ## n is the ins_id in a video
                        ...
                        if n in map_instance_cat.keys():
                            ...
                            print("ERROR: in video {}, instance:{} is dropped?".format(j + 1, n))
                        ...
                        if score_times > 0:
                            ...
                            if dict_ele["score"] >= 1.0 or dict_ele["score"] <= 0.0:
                                print("Error: video:{}, instance:{}'s score not in (0, 1)".format(j + 1, n))
                        else:
                            print("Error: video:{}, instance:{} has no scores".format(j + 1, n))
                        mAP_eval_results.append(dict_ele)
            print("res transfer costs {}s\n".format(time.time() - start))

            ## YouTube-VIS format per video annotation
            print("\nstart inputing per video anno")
            start = time.time()
            import json
            if "amodal" in self.ann_file:
                ...
                if "cut" in self.ann_file:
                    ...
                    dataset = json.load(
                        open("/beegfs/work/shared/kangdong_shared/sailvos_cut_json/png_amodal/valid_less0.75_png_amodal_per_video.json", 'r'))

                # for SAIL-VOS amodal video-level evaluation
                else:
                    print('\n\n------------SAIL-VOS amodal video-level evaluation------------\n')
                    dataset = json.load(
                        open("/beegfs/work/shared/kangdong_shared/sailvos_complete_json/png_amodal_cmplt_video/valid_less0.75_png_amodal_cmplt_vid_per_video.json", 'r'))
                ...
                if "cut" in self.ann_file:
                    ...
                    dataset = json.load(
                        open("/beegfs/work/shared/kangdong_shared/sailvos_cut_json/png_visible/valid_less0.75_png_visible_per_video.json", 'r'))

                # for SAIL-VOS modal video-level evaluation
                else:
                    print('\n\n------------SAIL-VOS modal video-level evaluation------------\n')
                    dataset = json.load(
                        open("/beegfs/work/shared/kangdong_shared/sailvos_complete_json/png_visible_cmplt_video/valid_less0.75_png_visible_cmplt_vid_per_video.json", 'r'))

            else:
                print("ERROR: no 'amodal' or 'visible' in ann_file")
                exit()

            print("inputing per video anno costs {}s\n".format(time.time() - start))

            from mmtrack.core.evaluation.eval_vis import eval_vis
            vis_eval_results = eval_vis(
                test_results=mAP_eval_results,
                vis_anns=dataset,
                maxDets=[1, 10, 100])
            eval_results.update(vis_eval_results)
            res_total["per video segm mAP"] = vis_eval_results
