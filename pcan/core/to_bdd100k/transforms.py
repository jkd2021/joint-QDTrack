...
def seg_track_to_bdd100k(dataset, results, out_base, nproc=4):
    ...
    results_track_0 = results['track_result'][:1600]
    img_names_0 = [
        dataset.data_infos[idx]['file_name']
        for idx in range(1600)
    ]

    results_track_1 = results['track_result'][1600:3200]
    img_names_1 = [
        dataset.data_infos[idx]['file_name']
        for idx in range(1600, 3200)
    ]

    results_track_2 = results['track_result'][3200:4800]
    img_names_2 = [
        dataset.data_infos[idx]['file_name']
        for idx in range(3200, 4800)
    ]

    results_track_3 = results['track_result'][4800:]
    img_names_3 = [
        dataset.data_infos[idx]['file_name']
        for idx in range(4800, 6475)
    ]
    results_track_list = [results_track_0, results_track_1, results_track_2, results_track_3]
    img_names_list = [img_names_0, img_names_1, img_names_2, img_names_3]
    for i in range(4):
        mask_merge_parallel(results_track_list[i], img_names_list[i], bitmask_base,
                            nproc)
