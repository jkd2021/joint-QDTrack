...
class QuasiDenseSegFeatEmbedTracker(QuasiDenseEmbedTracker):
    """
    That is (Amodal)PCAN Tracker
    """
    def update_memo(self, ids, bboxes, mask_preds, mask_feats, refine_preds,
                    embeds, labels, frame_id):
        tracklet_inds = ids > -1

        # update memo
        for id, bbox, mask_pred, mask_feat, refine_pred, embed, label in (
            zip(
                ids[tracklet_inds],
                bboxes[tracklet_inds],
                mask_preds[tracklet_inds],
                mask_feats[tracklet_inds],
                refine_preds[tracklet_inds],
            ...
            if id in self.tracklets.keys():
                ...
                self.tracklets[id]['mask_feat'] = mask_feat
                self.tracklets[id]['refine_pred'] = refine_pred
        ...
        self.backdrops.insert(
            ...
            dict(
                ...
                mask_feats=mask_feats[backdrop_inds],
                refine_preds=refine_preds[backdrop_inds],
    ...
    def memo(self):
        ...
        for k, v in self.tracklets.items():
            ...
            memo_mask_feats.append(v['mask_feat'][None, :])
            memo_refine_preds.append(v['refine_pred'][None, :])
        ...
        for backdrop in self.backdrops:
            ...
            memo_mask_feats.append(backdrop['mask_feats'])
            memo_refine_preds.append(backdrop['refine_preds'])
        ...
        memo_mask_feats = torch.cat(memo_mask_feats, dim=0)
        memo_refine_preds = torch.cat(memo_refine_preds, dim=0)
        memo_embeds = torch.cat(memo_embeds, dim=0)
        memo_labels = torch.cat(memo_labels, dim=0).squeeze(1)
        memo_vs = torch.cat(memo_vs, dim=0)
        return (
            memo_bboxes, memo_labels, memo_mask_preds, memo_mask_feats,
            memo_refine_preds, memo_embeds, memo_ids.squeeze(0), memo_vs)
    ...
    def match(self, bboxes, labels, masks, track_feats, frame_id, asso_tau=-1):
        ...
        mask_feats = masks['mask_feats'][inds, :]
        ...
        mask_feats = mask_feats[valids, :]
        embeds = embeds[valids, :]

        # init ids container
        ids = torch.full((bboxes.size(0), ), -1, dtype=torch.long)
        ref_feats = mask_feats.clone()
        ref_masks = mask_preds.clone()

        # match if buffer is not empty
        if bboxes.size(0) > 0 and not self.empty:
            (memo_bboxes, memo_labels, memo_mask_preds, memo_mask_feats,
             memo_refine_preds, memo_embeds, memo_ids, memo_vs) = self.memo
                ...
                if conf > self.match_score_thr:
                    if id > -1:
                        if bboxes[i, -1] > self.obj_score_thr:
                            ...
                            ref_feats[i] = memo_mask_feats[memo_ind]
                            #ref_masks[i] = memo_refine_preds[memo_ind]
                            ref_masks[i] = memo_mask_preds[memo_ind]
                        else:
        ...
        masks = dict(mask_pred=mask_preds, mask_feats=mask_feats)

        return bboxes, labels, masks, ids, embeds, ref_feats, ref_masks, inds, valids
