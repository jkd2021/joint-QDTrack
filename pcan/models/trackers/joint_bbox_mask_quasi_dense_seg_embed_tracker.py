...
class JointBBoxMaskQuasiDenseSegEmbedTracker(QuasiDenseEmbedTracker):
    """
    That is QDTrack-mots-joint+ Tracker.
    """
    def update_memo(self, ids, bboxes, bboxes_v, mask_preds_a, mask_preds_v, embeds, labels, frame_id):
        tracklet_inds = ids > -1
        # update memo
        for id, bbox, bbox_v, mask_pred_a, mask_pred_v, embed, label in (
            zip(
                ids[tracklet_inds],
                bboxes[tracklet_inds],
                bboxes_v[tracklet_inds],
            ...
            if id in self.tracklets.keys():
                ...
                self.tracklets[id]['bbox_v'] = bbox_v
        ...
        self.backdrops.insert(
            ...
            dict(
                ...
                bboxes_v=bboxes_v[backdrop_inds],
    ...
    def memo(self):
        ...
        for k, v in self.tracklets.items():
            ...
            memo_bboxes_v.append(v['bbox_v'][None, :])
        ...
        for backdrop in self.backdrops:
            ...
            memo_bboxes_v.append(backdrop['bboxes_v'])
        ...
        memo_bboxes_v = torch.cat(memo_bboxes_v, dim=0)
        ...
        return (
            memo_bboxes, memo_bboxes_v, memo_labels, memo_mask_preds_a, memo_mask_preds_v, memo_embeds,
            memo_ids.squeeze(0), memo_vs)

    def match(self, bboxes, bboxes_v, labels, masks_a, masks_v, track_feats, frame_id, asso_tau=-1):

        _, inds = bboxes[:, -1].sort(descending=True)
        bboxes = bboxes[inds, :]
        bboxes_v = bboxes_v[inds, :]
        labels = labels[inds]
        ...
        if bboxes.size(0) > 0 and not self.empty:
            (memo_bboxes, memo_bboxes_v, memo_labels, memo_mask_preds_a, memo_mask_preds_v, memo_embeds, memo_ids,
             memo_vs) = self.memo
        ...
        self.update_memo(ids, bboxes, bboxes_v, mask_preds_a, mask_preds_v, embeds, labels, frame_id)
        masks_a = dict(mask_pred=mask_preds_a)
        masks_v = dict(mask_pred=mask_preds_v)
        del mask_preds_v, mask_preds_a
        torch.cuda.empty_cache()

        return bboxes, bboxes_v, labels, masks_a, masks_v, ids