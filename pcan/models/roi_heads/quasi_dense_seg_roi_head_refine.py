...
class QuasiDenseSegRoIHeadRefine(QuasiDenseSegRoIHead):
    ...
    def forward_train(self,
                      x,                # extracted features from backbone/neck of the current image
                      img_metas,
                      proposal_list,
                      gt_bboxes,
                      gt_labels,
                      ...
                      ref_x,            # extracted features from backbone/neck of the reference image
                      ref_img_metas,
                      ref_proposals,
                      ref_gt_bboxes,
                      ref_gt_labels,
        ...
        refine_results = self._refine_forward_train(                    # key for current image, ref for reference image
    ...
    def simple_test_refine(self, img_metas, key_feats, key_masks, key_bboxes,
        ...
        if key_bboxes.shape[0] == 0:
            ...
            select_inds = torch.where(key_labels >= 1000)  ## a bug difficult to find, whose ori value is 10, which
            refine_preds = refine_results['refine_pred']  ## makes the objs of class label >= 10 not be refined in test
