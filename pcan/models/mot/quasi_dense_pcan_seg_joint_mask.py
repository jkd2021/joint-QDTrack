...
class JointMaskQuasiDenseMaskRCNN(EMQuasiDenseFasterRCNN):
    ...
    def forward_test(self, img, img_metas, rescale=False):
        ...
        if track_feats is None:
            ...
            bboxes, labels, masks_a, masks_v, ids = self.tracker.match(
                bboxes=det_bboxes,
                labels=det_labels,
            ...
            _, segms_v, _ = self.roi_head.get_seg_masks(
                img_metas, bboxes, labels, masks_v, rescale=rescale)
            ...
            track_result_v = segtrack2result(bboxes, labels, segms_v, ids)
