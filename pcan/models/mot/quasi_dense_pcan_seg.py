...
class QuasiDenseMaskRCNN(EMQuasiDenseFasterRCNN):
    ...
    def forward_test(self, img, img_metas, rescale=False):           # this is (Amodal)QDTrack-mots main test function
    ...
    def show_result(self,
        ...
        for id, item in track_result.items():
            ...
            label_text = 'instance_id:{}, class:{}, conf:{:.2}'.format(int(id), self.CLASSES[int(item['label'])], item['bbox'].tolist()[-1])
            bg_height = 12
            bg_width = 7
            bg_width = len(label_text) * bg_width
            if left_top[1] - bg_height > 0 and left_top[0] + bg_width < img.shape[1]:
                plt.gca().add_patch(
            ...
            elif left_top[1] - bg_height <= 0 and left_top[0] + bg_width < img.shape[1]:
                plt.gca().add_patch(
                ...
                plt.gca().add_patch(
                    Rectangle((img.shape[1] - bg_width, left_top[1] - bg_height),
                              bg_width,
                              bg_height,
                              edgecolor=color,
                              facecolor=color))
                plt.text(img.shape[1] - bg_width, left_top[1], label_text, fontsize=5)
            else:
                plt.gca().add_patch(
                    Rectangle((img.shape[1] - bg_width, left_top[1]),
                              bg_width,
                              bg_height,
                              edgecolor=color,
                              facecolor=color))
                plt.text(img.shape[1] - bg_width, left_top[1] + bg_height, label_text, fontsize=5)
