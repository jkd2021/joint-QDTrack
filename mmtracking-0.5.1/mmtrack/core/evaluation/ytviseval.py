...
class YTVISeval:
    ...
    def _prepare(self):
        ...
        def _toMask(anns, coco):
            ...
            for ann in anns:
                ...
                # calculate the occlusion score per instance in a video
                if 'occlude_rate' in ann:
                    score_list = [a for a in ann['occlude_rate'] if a is not None]
                    # print(" score_list",score_list)
                    score_list.sort()
                    # print(" score_list2",score_list)
                    top_score_list = score_list[-math.ceil(len(score_list) / 2):]   # select only top 50% scores
                    ann['occ_score'] = sum(top_score_list) / len(top_score_list)   # because some ins may suffer
                    # print(" ann['occ_score'] 2 ", ann['occ_score'])              # very few frame with high occ
                                                                                   # which should be high socre?
        p = self.params
        if p.useCats:
    ...
    def evaluate(self):
        ...
        self.evalImgs = [
            evaluateVid(vidId, catId, areaRng, occRng, maxDet) for catId in catIds
            for areaRng in p.areaRng for occRng in p.occRng for vidId in p.vidIds
        ]
        ...
        for g in gt:
            if g['ignore'] or (g['avg_area'] < aRng[0]
                               or g['avg_area'] > aRng[1])\
                                    or (g['occ_score'] < oRng[0] or g['occ_score'] > oRng[1]):
                g['_ignore'] = 1
            else:
                g['_ignore'] = 0
    ...
    def accumulate(self, p=None):
        ...
        M = len(p.maxDets)
        precision = -np.ones(
            (T, R, K, A,O, M))  # -1 for the precision of absent categories
    ...
    def summarize(self):
        ...
        def _summarize(ap=1, iouThr=None, areaRng='all', occRng='all', maxDets=100):
            ...
            if ap == 1:
                # dimension of precision: [TxRxKxAxOxM] # xO
                ...
                # dimension of recall: [TxKxAxOxM] # xO
                s = self.eval['recall']
                if iouThr is not None:
                    t = np.where(iouThr == p.iouThrs)[0]
                    s = s[t]
            ...
            if ap == 1 and iouThr is None and areaRng == 'all':
                for i in range(s.shape[2]):
                    s_cls = s[:, :, i, :]
                    mean_s_cls = np.mean(s_cls[s_cls > -1])
                    print('AP of cls {} {}: '.format(i+1, self.cocoGt.cats[i+1]["name"]) + str(mean_s_cls))
            ...
            print(
                iStr.format(titleStr, typeStr, iouStr, areaRng, occRng, maxDets,
                            mean_s))
            return mean_s
        ...
        def _summarizeDets():
            ...
            stats[3] = _summarize(
                1, areaRng='small', iouThr=.5, maxDets=self.params.maxDets[2])
            stats[4] = _summarize(
                1, areaRng='medium', iouThr=.5, maxDets=self.params.maxDets[2])
            stats[5] = _summarize(
                1, areaRng='large', iouThr=.5, maxDets=self.params.maxDets[2])

            stats[6] = _summarize(1, iouThr=.5, occRng='partial', maxDets=self.params.maxDets[2])
            stats[7] = _summarize(1, iouThr=.5, occRng='medium', maxDets=self.params.maxDets[2])
            stats[8] = _summarize(1, iouThr=.5, occRng='heavy', maxDets=self.params.maxDets[2])
            ...
            stats[15] = _summarize(0, iouThr=.5, occRng='partial', maxDets=self.params.maxDets[2])
            stats[16] = _summarize(0, iouThr=.5, occRng='medium', maxDets=self.params.maxDets[2])
            stats[17] = _summarize(0, iouThr=.5, occRng='heavy', maxDets=self.params.maxDets[2])
...
class Params:
    ...
    def setDetParams(self):
        ...
        self.areaRng = [[0 ** 2, 1e5 ** 2], [0 ** 2, 32 ** 2], [32 ** 2, 96 ** 2],  # according to the paper of SAIL-VOS
                        [96 ** 2, 1e5 ** 2]]                                        # the area range is [0**2, 32**2,
        # self.areaRng = [[0**2, 1e5**2], [0**2, 128**2], [128**2, 256**2],         # 96**2, 1e5**2]
        #                 [256**2, 1e5**2]]
        self.areaRngLbl = ['all', 'small', 'medium', 'large']

        self.occRng = [[0, 0.75], [0, 0.25], [0.25, 0.5], [0.25, 0.75]]
        self.occRngLbl = ['all', 'partial', 'medium', 'heavy']
