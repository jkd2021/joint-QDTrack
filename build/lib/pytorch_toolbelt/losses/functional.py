...
def tversky_score(y_pred: torch.Tensor, y_true: torch.Tensor, alpha, smooth=0, eps=1e-7, dims=None
) -> torch.Tensor:

    assert y_pred.size() == y_true.size()

    if dims is not None:
        true_pos = torch.sum(y_pred * y_true, dim=dims)
        false_neg = torch.sum(y_true * (1-y_pred), dim=dims)
        false_pos = torch.sum((1-y_true)*y_pred, dim=dims)
    else:
        true_pos = torch.sum(y_pred * y_true)
        false_neg = torch.sum(y_true * (1-y_pred))
        false_pos = torch.sum((1-y_true)*y_pred)

    return (true_pos + smooth)/(true_pos + alpha*false_neg + (1-alpha)*false_pos + smooth + eps)
