...
from pcan.models.losses import DiceLoss
...
class BoundFCNMaskHeadPlus(FCNMaskHead):
    ...
    def loss(self, mask_pred, mask_targets, labels):
        ...
        if self.class_agnostic:
            ...
            indices = torch.arange(mask_pred.shape[0])
            mask_pred = mask_pred[indices, list(labels.cpu().numpy())]
            loss_dice = self.loss_dice(mask_pred, mask_targets)


        #print('loss mask:', loss_mask.shape)
        loss['loss_mask'] = loss_mask
        loss_d['loss_dice'] = loss_dice
        return loss, loss_d
    ...
    def loss_bound(self, bound_pred, bound_targets, labels):
        ...
        if self.class_agnostic:
            ...
            indices = torch.arange(bound_pred.shape[0])
            bound_pred = bound_pred[indices, list(labels.cpu().numpy())]
